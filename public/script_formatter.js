...
var lf_version = '2.12.0';
var lf_release_date = '06/04/2024'
            ...
            Object.keys(dictionaryCache).forEach(language => {
                Object.entries(dictionaryCache[language]).forEach(([word, description]) => {
                    ...
                    row.innerHTML = `
                        ...
                        <td>${description}</td>
                        <td>${language}</td>
                        <td><span class="delete_button" onclick="deleteRow(this)">Delete</span></td>
                    `;
...
/* POPULA AS TABELAS DO RASCUNHO COM OS DADOS EM CACHÊ  */
    ...
    function populateDraftsPopupFromCache() {
        ...
        if (cacheData) {
            ...
            // Preenche o corpo da tabela com os dados do cache
            Object.keys(draftsCache).forEach(trackId => {
                const draft = draftsCache[trackId];
                const row = document.createElement('tr');
                row.innerHTML = `
                    <td>${new Date(draft.datetime).toLocaleDateString()}</td>
                    <td>${draft.track_name}</td>
                    <td>${draft.artist_name}</td>
                    <td><span class="table_svg_icon" onclick="copyTranscriptionFromCache('${trackId}')">
                    ...
                    <td><span class="table_svg_icon" onclick="draftStartPlayback('${draft.track_id}')">
                    ...
                    <td><span class="table_svg_icon" onclick="exportDraft('${trackId}')">
                    ...
                    <td><span class="table_svg_icon" onclick="deleteDraft('${trackId}')">
    ...
    function copyTranscriptionFromCache(trackId) {
        // Recupera os dados do cache
        const cacheData = localStorage.getItem('localDrafts');
    ...
    function exportAllDrafts() {
        ...
        if (cacheData) {
            ...
            if (Object.keys(draftsCache).length > 0) {
                ...
                // Define o nome do arquivo
                const fileName = 'all_drafts.json';
                link.download = fileName;
